...
The default browser stylesheet is the set of rules that browsers have to apply
some minimum style to elements.

Most of the time, those styles are very useful. Since every browser has its own set, it's common to find a common ground.

Rather than removing all defaults, like one of the CSS reset approaches does,
